    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.type == "date_histogram":
            ...
            if self.order_by_metric:
                json_data["params"]["orderBy"] = {
                    "type": "column",
                    "columnId": "placeholder",
                }  # Need to map order_by_metric to columnId
...
class DataSourceState(BaseModel):
    formBased: Dict[str, Dict[str, LayerDataSourceState]] = Field(
        default_factory=dict
    )  # Structure: formBased -> layers -> {layerId: LayerDataSourceState}
...
class XYVisualizationState(LensVisualizationState):
    ...
    gridlinesVisibilitySettings: Dict[str, bool] = Field(
        default_factory=lambda: {"x": True, "yLeft": True, "yRight": True}
    )  # Needs adjustment for line
...
class LensPanel(Panel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        markdown_panel = base_panel | lens_panel_config
