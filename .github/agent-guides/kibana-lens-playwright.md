...
- **Save snapshots to disk, not inline.** Use `browser_snapshot(filename="/tmp/gh-aw/agent/step_name.md")` to write the accessibility tree to a file. This keeps your context window small — inline snapshots are 8-17K chars each and accumulate fast. Then use `grep` on the saved file to find what you need:
  ```bash
  # Find a button ref
  grep 'button.*Legend\|button.*Save\|button.*Close' /tmp/gh-aw/agent/step_name.md

  # Find a dropdown option
  grep 'option.*Extra large' /tmp/gh-aw/agent/step_name.md

  # Find a combobox
  grep 'combobox.*Field' /tmp/gh-aw/agent/step_name.md
  ```
- **Note: `browser_click` and `browser_wait_for` return inline diffs.** You can't avoid these, but they're usually small (changed elements only). Don't take an extra `browser_snapshot()` after a click just to see refs — the click response already contains them. Only save a new disk snapshot when you need to search a large page.
...
- **Keep `browser_run_code` return values small.** When making API calls via `page.request`, extract only what you need:
  ...
  ```
- **Comboboxes are NOT `<select>`.** Kibana uses EUI comboboxes. The pattern is:
  1. `browser_type` into the combobox ref to filter options
  2. `browser_snapshot(filename="/tmp/gh-aw/agent/combo_options.md")` to save options to disk
  3. `grep 'option.*your_value' /tmp/gh-aw/agent/combo_options.md` to find the ref
  4. `browser_click` the option ref
  Never use `browser_fill_form` with type `combobox`.
