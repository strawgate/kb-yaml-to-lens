...
class TestKibanaClientAsync:
    ...
    async def test_find_dashboard_by_title_single_match(self) -> None:
        ...
        mock_response.json = AsyncMock(
            return_value={
                'saved_objects': [
                    {'id': 'dashboard-123', 'type': 'dashboard', 'attributes': {'title': 'System Metrics'}},
                ],
                'total': 1,
            }
        )
        ...
        # Test
        dashboard_id = await client.find_dashboard_by_title('System Metrics')
    ...
    async def test_find_dashboard_by_title_no_matches(self) -> None:
        ...
        # Test
        dashboard_id = await client.find_dashboard_by_title('Non Existent Dashboard')
    ...
    async def test_find_dashboard_by_title_multiple_matches(self) -> None:
        ...
        # Mock the response with multiple matches
        mock_response = AsyncMock()
        mock_response.status = 200
        mock_response.json = AsyncMock(
            return_value={
                'saved_objects': [
                    {'id': 'dashboard-first', 'type': 'dashboard', 'attributes': {'title': 'Metrics Dashboard'}},
                    {'id': 'dashboard-second', 'type': 'dashboard', 'attributes': {'title': 'Metrics Dashboard Copy'}},
                    {'id': 'dashboard-third', 'type': 'dashboard', 'attributes': {'title': 'Metrics Dashboard v2'}},
                ],
                'total': 3,
            }
        )
        ...
        # Test
        dashboard_id = await client.find_dashboard_by_title('Metrics')

        # Verify - should return the first match
    ...
    async def test_find_dashboard_by_title_with_url_encoded_spaces(self) -> None:
        ...
        mock_response.json = AsyncMock(
            return_value={
                'saved_objects': [
                    {'id': 'dashboard-with-spaces', 'type': 'dashboard', 'attributes': {'title': 'System Performance Metrics'}},
                ],
                'total': 1,
            }
        )
        ...
        # Test with spaces in the title
        dashboard_id = await client.find_dashboard_by_title('System Performance Metrics')

        # Verify
        assert dashboard_id == 'dashboard-with-spaces'
        client._get.assert_called_once()

        # Verify the params passed to _get include the search term
        call_args = client._get.call_args
        assert call_args is not None
        assert 'params' in call_args.kwargs
        assert call_args.kwargs['params']['search'] == 'System Performance Metrics'
