...
def compile_options_list_control(order: int, *, control: OptionsListControl) -> tuple[KbnOptionsListControl, KbnReference]:
    ...
    data_view_ref_name = f'{control_id}:optionsListDataView'
    ...
    kbn_control = KbnOptionsListControl(
        ...
        explicitInput=KbnOptionsListControlExplicitInput(
            id=control_id,
            dataViewRefName=data_view_ref_name,
...
def compile_range_slider_control(order: int, *, control: RangeSliderControl) -> tuple[KbnRangeSliderControl, KbnReference]:
    ...
    """
    control_id = control.get_id()
    data_view_ref_name = f'{control_id}:rangeSliderDataView'
...
def compile_control(order: int, *, control: ControlTypes) -> tuple[KbnControlTypes, KbnReference | None]:  # noqa: PLR0911
    ...
    Returns:
        tuple: A tuple containing the compiled Kibana control view model and an optional reference.
               Controls that use data views return a reference; others return None.
...
@log_compile
def compile_control_panels_json(
    controls: Sequence[ControlTypes],
) -> tuple[KbnControlPanelsJson, list[KbnReference]]:
    ...
    Returns:
        tuple: A tuple containing the compiled panels JSON and a list of references.
    ...
    for i, config_control in enumerate(iterable=controls):
        ...
        if reference is not None:
            kbn_references.append(reference)

    return kbn_control_panels_json, kbn_references
...
def compile_control_group(
    *, control_settings: ControlSettings, controls: Sequence[ControlTypes]
    ...
    Returns:
        tuple: A tuple containing the compiled control group input and a list of references.
