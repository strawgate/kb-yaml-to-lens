...
When using the array format, query parts are automatically joined with the ES|QL pipe operator (`|`). This enables powerful query reuse patterns with YAML anchors—see [ES|QL Query Reuse with YAML Anchors](#esql-query-reuse-with-yaml-anchors) below.
