...
from dashboard_compiler.panels.auto_layout import (
    AutoLayoutEngine,
    BlockedEngine,
...
class TestAutoLayoutEngineLegacy:
    """Test suite for legacy AutoLayoutEngine batch API."""
    ...
    def test_blocked_algorithm(self) -> None:
        """Test blocked algorithm never fills gaps above."""
        engine = AutoLayoutEngine(algorithm='blocked')
        engine.mark_locked_panel(0, 0, 24, 20)
        positions = engine.compute_positions(
            [
                (0, 24, 12),
        ...
        # Unlike up-left, second panel goes to y=20 not y=12
        assert positions == [(24, 0), (0, 20)]

    def test_first_available_gap_algorithm(self) -> None:
        """Test first-available-gap algorithm fills earliest gaps."""
        engine = AutoLayoutEngine(algorithm='first-available-gap')
        engine.mark_locked_panel(0, 0, 24, 20)
        positions = engine.compute_positions(
            [
                (0, 24, 12),
    ...
    def test_panel_exceeds_grid_width(self) -> None:
        """Test that panel wider than grid raises ValueError."""
        engine = AutoLayoutEngine(algorithm='up-left')
        with pytest.raises(ValueError, match=r'Panel width .* exceeds grid width'):
