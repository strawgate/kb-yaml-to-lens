...
async def test_compile_control_settings(config: dict, desired_output: dict) -> None:
    ...
    if 'ignoreParentSettingsJSON' in kbn_control_group_input_as_dict and isinstance(
        kbn_control_group_input_as_dict['ignoreParentSettingsJSON'], str
    ):
        kbn_control_group_input_as_dict['ignoreParentSettingsJSON'] = json.loads(
            kbn_control_group_input_as_dict['ignoreParentSettingsJSON']
        )

    if 'panelsJSON' in kbn_control_group_input_as_dict and isinstance(kbn_control_group_input_as_dict['panelsJSON'], str):
        kbn_control_group_input_as_dict['panelsJSON'] = json.loads(kbn_control_group_input_as_dict['panelsJSON'])
