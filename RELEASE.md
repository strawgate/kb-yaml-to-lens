...
# 2. Commit and tag
git add packages/kb-dashboard/pyproject.toml vscode-extension/package.json pyproject.toml
...
cd packages/kb-dashboard && make build && make publish

# Docker (multi-arch - prefer re-running workflow)
# Manual single-arch build for testing only:
cd packages/kb-dashboard && docker build -t ghcr.io/strawgate/kb-yaml-to-lens/kb-dashboard-compiler:1.0.0 .
