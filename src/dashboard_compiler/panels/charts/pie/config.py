...
class LensPieChart(BasePieChart):
    ...
    """The primary dimensions that determine the slices of the pie chart."""

    secondary_slice_by: list[LensDimensionTypes] | None = Field(default=None)
    """Optional secondary dimensions for hierarchical/nested slicing."""

    collapse_fns: dict[str, str] | None = Field(default=None)
    """Collapse functions for dimensions. Maps dimension ID to collapse function (sum, avg, min, max)."""
...
class ESQLPieChart(BasePieChart):
    ...
    secondary_slice_by: list[ESQLDimensionTypes] | None = Field(default=None)
    """Optional secondary dimensions for hierarchical/nested slicing."""
