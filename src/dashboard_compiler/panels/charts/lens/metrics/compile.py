...
# Type alias for compile_lens_metric return value:
# - str: Primary metric ID (used in visualization state, column ordering)
# - KbnLensMetricColumnTypes: Primary column (user-facing metric)
# - dict[str, KbnLensMetricColumnTypes]: Helper columns (math + aggregations for formulas, empty dict for simple metrics)
CompiledMetricResult = tuple[str, KbnLensMetricColumnTypes, dict[str, KbnLensMetricColumnTypes]]
...
def collect_aggregations(
    ...
    # All LensFormulaOperations types have been handled above
    msg = f'Unsupported formula operation type: {type(operation)}'  # pyright: ignore[reportUnreachable]
    raise NotImplementedError(msg)


def compile_lens_metric(metric: LensMetricTypes) -> CompiledMetricResult:  # noqa: PLR0912, PLR0915
    """Compile a single LensMetricTypes object into its Kibana view model.

    Formula metrics generate multiple columns (formula, math, aggregations) which are
    all returned alongside the primary column.

    Args:
        metric (LensMetricTypes): The LensMetricTypes object to compile.

    Returns:
        tuple[str, KbnLensMetricColumnTypes, dict[str, KbnLensMetricColumnTypes]]:
            - Primary metric ID (for visualization state, column ordering)
            - Primary column (user-facing metric)
            - Helper columns (math + aggregations for formulas, empty dict for simple metrics)
    ...
    if isinstance(metric, LensFormulaMetric):
        ...
        # Formula metrics require multiple columns: the primary formula column,
        # the math column (with TinymathAST), and all aggregation columns
        return (
            base_metric_id,
            formula_column,
            {
...
def compile_lens_metrics(metrics: Sequence[LensMetricTypes]) -> dict[str, KbnLensMetricColumnTypes]:
    ...
    for primary_id, primary_column, helper_columns in compiled_metrics:
        result_columns[primary_id] = primary_column
        result_columns.update(helper_columns)
