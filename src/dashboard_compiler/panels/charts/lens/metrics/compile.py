...
class CompiledMetricResult:
    """Result of compiling a metric, potentially with helper columns for formulas.

    Formula metrics require multiple columns in Kibana:
    - A primary formula column (user-facing)
    - A math column (contains the TinymathAST)
    - Aggregation columns (actual metric operations)

    This dataclass encapsulates all columns generated by a single metric compilation.
    """
    ...
    """The primary column (shown in the chart)."""

    helper_columns: dict[str, KbnLensMetricColumnTypes]
    """Additional columns required by formula metrics (math column + aggregations)."""
...
def compile_lens_metric(metric: LensMetricTypes) -> CompiledMetricResult:  # noqa: PLR0912, PLR0915
    ...
    all returned in the CompiledMetricResult structure.

    Args:
        metric (LensMetricTypes): The LensMetricTypes object to compile.

    Returns:
        CompiledMetricResult: The compiled metric with primary column and any helper columns.
...
def compile_lens_metrics(metrics: Sequence[LensMetricTypes]) -> dict[str, KbnLensMetricColumnTypes]:
    ...
    for result in compiled_metrics:
        result_columns[result.primary_id] = result.primary_column
        result_columns.update(result.helper_columns)
