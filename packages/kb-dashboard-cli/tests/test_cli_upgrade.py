...
""".strip()
    ...
    def test_upgrade_migrates_panel_size_and_position_aliases(self, tmp_path: Path) -> None:
        """Rewrites panel size/position aliases to canonical keys."""
        input_file = tmp_path / 'layout-legacy.yaml'
        input_file.write_text(
            """
dashboards:
  - id: layout-legacy
...
""".strip()
        ...
        assert panel['size']['w'] == 20
        assert panel['size']['h'] == 10
        assert 'width' not in panel['size']
        assert 'height' not in panel['size']
        assert panel['position']['x'] == 5
        assert panel['position']['y'] == 3
        assert 'from_left' not in panel['position']
        assert 'from_top' not in panel['position']

    def test_upgrade_migrates_grid_aliases(self, tmp_path: Path) -> None:
        """Rewrites grid aliases to canonical x/y/w/h keys."""
        input_file = tmp_path / 'grid-legacy.yaml'
        input_file.write_text(
            """
dashboards:
  - id: grid-legacy
...
""".strip()
        ...
        assert panel['grid']['x'] == 7
        assert panel['grid']['y'] == 4
        assert panel['grid']['w'] == 18
        assert panel['grid']['h'] == 6
        assert 'from_left' not in panel['grid']
        assert 'from_top' not in panel['grid']
        assert 'width' not in panel['grid']
        assert 'height' not in panel['grid']
