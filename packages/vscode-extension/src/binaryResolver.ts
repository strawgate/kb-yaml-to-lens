...
export class BinaryResolver {
    ...
    resolveLSPServer(outputChannel?: vscode.OutputChannel): BinaryResolverResult {
        ...
        // For development, use workspace root so kb_dashboard can be imported
        const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        const cwd = workspaceRoot ?? path.dirname(this.extensionPath);

        outputChannel?.appendLine(`Using local Python LSP server: ${pythonPath} -m kb_dashboard.lsp.server`);
        outputChannel?.appendLine(`Working directory: ${cwd}`);

        return {
            executable: pythonPath,
            args: ['-m', 'kb_dashboard.lsp.server'],
            cwd,
            isBundled: false
        };
    }
