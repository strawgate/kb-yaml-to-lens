"""Rule: metrics-* ES|QL sources require Kibana/Elasticsearch 9.2+."""
...
SOURCE_PATTERN = re.compile(r'^\s*(?P<command>TS|FROM)\s+(?P<source>\S+)', re.IGNORECASE)
...
class ESQLTSMetricsMinVersionRule(ChartRule[ESQLConfig, EmptyOptions]):
    ...
    description: str = 'metrics-* ES|QL sources require Kibana/Elasticsearch 9.2+'
    ...
    def check_chart(  # pyright: ignore[reportImplicitOverride]
        ...
        """Check ES|QL panel for metrics-* source usage that requires 9.2+."""
        ...
        if source.startswith('metrics-'):
            return Violation(
                rule_id=self.id,
                message=(f'{command} queries on metrics-* require Kibana/Elasticsearch 9.2+'),
