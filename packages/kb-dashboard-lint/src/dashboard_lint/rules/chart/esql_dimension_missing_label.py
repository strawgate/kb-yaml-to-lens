"""Rule: ES|QL datatable dimensions should have explicit labels."""
...
class ESQLDimensionMissingLabelRule(ChartRule[ESQLDatatablePanelConfig, EmptyOptions]):
    """Rule: ES|QL datatable dimensions should have explicit labels.
    ...
    id: str = 'esql-dimension-missing-label'
    description: str = 'ES|QL datatable dimensions should have explicit labels'
    ...
    def check_chart(  # pyright: ignore[reportImplicitOverride]
        ...
        """Check ES|QL datatable for dimensions without labels.
        ...
        Returns:
            List of violations for dimensions missing labels, or None if all have labels.
        ...
        for idx, dimension in enumerate(config.dimensions):
            if dimension.label is None or len(dimension.label) == 0:
                violations.append(
                    Violation(
                        rule_id=self.id,
                        message=f"ES|QL dimension '{dimension.field}' should have an explicit label for better readability",
