"""View models for waffle chart visualization state in Kibana JSON format.

Waffle charts in Kibana use the lnsPie visualization type with shape: 'waffle'.
These models represent the compiled structure sent to Kibana for configuring
waffle chart layers in Lens visualizations.

Fixture Examples:
    https://github.com/strawgate/kb-yaml-to-lens-fixtures
    - ES|QL: output/<version>/waffle-esql.json
    - Data View: output/<version>/waffle-dataview.json
"""
...
class KbnWaffleStateVisualizationLayer(KbnBaseStateVisualizationLayer):
    """View model for waffle chart visualization layer configuration.

    Represents a data layer in a waffle chart visualization. Defines the metrics
    ...
    """Always 'data' for waffle chart layers."""
    ...
    """Whether to allow multiple metrics. Should be False for waffle charts; omitted when None."""
...
class KbnWaffleVisualizationState(KbnBaseStateVisualization):
    """View model for waffle chart visualization state after compilation to Kibana Lens format.

    This model represents the complete visualization state for waffle charts as stored
    in the Kibana saved object JSON structure. Waffle charts use the lnsPie visualization
    type with shape set to 'waffle'.

    The visualization state is part of the larger Lens panel configuration and defines how
    the waffle chart should be rendered in Kibana dashboards.
    ...
    shape: Literal['waffle'] = 'waffle'
    """Shape of the chart. Always 'waffle' for waffle charts."""

    layers: list[KbnWaffleStateVisualizationLayer] = Field(...)
    """List of data layers for the waffle chart."""
