...
| `appearance.color.apply_to` | `Literal['value', 'background']` | Controls where metric colors are applied. | `'background'` | No |
| `appearance.color.strategy` | `Literal['none', 'value_mapping'] \| None` | Controls whether value-mapping colors are active. If omitted, inferred from `color`. | `None` | No |
| `color_mode` | `Literal['value', 'background']` | Legacy alias for `appearance.color.apply_to`. | `'background'` | No |
...
Use `appearance.color.apply_to` to control how metric colors are applied:
...
`appearance.color.strategy` can be used to make color behavior explicit:

- `value_mapping`: use configured `color` value mappings
- `none`: disable metric color strategy
